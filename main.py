...
def transcreve_tab_mic():
    ...
    while True:
        if webrtx_ctx.audio_receiver:
            ...
            if len (chunk_audio) > 0 and agora - tempo_ultima_transcricao > 10:
...
def transcreve_tab_video():
    prompt_input = st.text_input('(Opcional) Adicione o seu Prompt', key='input_video')
    arquivo_video = st.file_uploader('Adicione um arquivo de áudio .mp4', type=['mp4'])
...
def transcreve_tab_audio():
    prompt_input = st.text_input('(Opcional) Adicione o seu Prompt', key='input_audio')
...
def main():
    st.header('Bem-vindo ao EchoAI', divider=True)
    st.markdown('#### Transcreva áudio em texto a partir do microfone, vídeos ou arquivos de áudio.')
